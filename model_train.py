...
def get_metric_score(model: ReviewNet, metric: str, test_loader: DataLoader) -> float:
    ...
    with torch.no_grad():
        ...
        for X_batch, y_batch in test_loader:
            ...
            metric_score = metric_fn.compute(predictions=y_pred, references=y_batch)
            scores.append(metric_score)
    return np.sum(scores) / len(scores)

def train(train_loader: DataLoader, test_loader: DataLoader, metric: str='f1', num_epochs: int=3):
    net = ReviewNet()
